...
class DbClientApp:
    def __init__(self, root):
        ...
        # NEW: Connection status label for DB1
        ...
        # NEW: Connection status label for DB2
        self.status2_label = ttk.Label(frm_db2, text="Not connected", foreground="red")
        self.status2_label.grid(row=4, column=0, columnspan=4, pady=2)
    ...
    def _get_dp_credentials(self):
        """Return (host, port, service, user, password) for the selected DP target."""
    ...
    def _get_dp_engine(self):
        """Return the SQLAlchemy engine for the selected DP target."""
        target = self.dp_db_target.get()
    ...
    def generate_datapump_cmd(self):
        ...
        # Auto filenames for export (based on first schema)
        if self.dp_mode.get() == "export":
