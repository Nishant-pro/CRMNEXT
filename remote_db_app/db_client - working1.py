...
class DbClientApp:
    ...
    def _build_datapump_tab(self):
        ...
        # --- Remap Schema (import mode) ---
        row += 1
        ttk.Label(frm_main, text="Remap Schema:").grid(row=row, column=0, sticky=tk.W, pady=2)
        self.remap_from = ttk.Entry(frm_main, width=15)
        self.remap_from.grid(row=row, column=1, pady=2, padx=5, sticky=tk.W)
        ttk.Label(frm_main, text="From").grid(row=row, column=2, padx=(0,5), sticky=tk.W)
        self.remap_to = ttk.Entry(frm_main, width=15)
        self.remap_to.grid(row=row, column=3, pady=2, padx=5, sticky=tk.W)
        ttk.Label(frm_main, text="To").grid(row=row, column=4, padx=(0,5), sticky=tk.W)
        ...
        ttk.Label(frm_transfer, text="Source (DB Server) SSH:").grid(row=0, column=0, columnspan=5, sticky=tk.W)
        ...
        self.source_path.grid(row=3, column=1, columnspan=4, padx=5, pady=2, sticky="ew")
        ttk.Button(frm_transfer, text="Auto-fill", command=self.autofill_source_path).grid(row=3, column=5, padx=5)
        ...
        self.dest_path.grid(row=7, column=1, columnspan=4, padx=5, pady=2, sticky="ew")

        self.transfer_btn = ttk.Button(frm_transfer, text="Transfer Files", command=self.transfer_files)
        self.transfer_btn.grid(row=8, column=0, columnspan=6, pady=10)
    ...
    def generate_datapump_cmd(self):
        try:
            ...
            if not all([conn_user, conn_pw, conn_host, conn_port, conn_service]):
                messagebox.showwarning("Missing connection", "Fill in all DB1 connection fields.")
                return
    ...
    def run_query(self):
        if self.engine is None:
            messagebox.showwarning("Not connected", "Please connect to DB1 first.")
            return
    ...
    def compare_objects(self):
        ...
        def get_objects(engine, obj_type):
            """Return set of object names and count for a given type."""
            try:
        ...
        # Perform comparison in a thread to keep UI responsive
        def compare_thread():
