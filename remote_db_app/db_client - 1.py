...
class DbClientApp:
    ...
    def _build_datapump_tab(self):
        # Outer frame with scrollbar (in case it gets too tall)
        ...
        # Configure grid weights for the whole scrollable frame
    ...
    def generate_datapump_cmd(self):
        """Build the expdp/impdp command from fields and display it."""
        try:
    ...
    def run_datapump(self):
        """Execute the generated command in a separate thread."""
    ...
    def autofill_source_path(self):
        """Construct source path from directory object name (you still need to know the physical path)."""
        # The Oracle directory object doesn't give us the OS path automatically.
        # We'll just prefill with a common pattern: /u01/app/oracle/admin/{service_name}/dpdump/
        service = self.db_name.get().strip()
        if service:
            proposed = f"/u01/app/oracle/admin/{service}/dpdump/"
        else:
            proposed = "/u01/app/oracle/admin/orcl/dpdump/"
        self.source_path.delete(0, tk.END)
        self.source_path.insert(0, proposed)

    def transfer_files(self):
        """Copy dumpfile and logfile from source server to destination server using SFTP."""
        dumpfile = self.dumpfile_entry.get().strip()
        logfile = self.logfile_entry.get().strip()
    ...
    def _log(self, message):
        """Append a line to the output_text."""
